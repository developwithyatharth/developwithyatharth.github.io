/* Velocity Runner: Rise of Bharat
   Combat + EMP Upgrade
   New:
   - F / Shoot button fires at drone
   - Drone respawns continuously
   - Drone shoots EMP attacks
   - EMP reduces Core Health
   - Core Health 0 = Game Over
*/

let scene, camera, renderer;
let player, suryaCore, drone;

let roadGroup, obstacleGroup, shardGroup, cityGroup, rainGroup, bulletGroup, empGroup;
...
function startGame() {
  ...
  if (runnerNameText) runnerNameText.textContent = runnerName;
...
function createRoad() {
  ...
  for (let i = 0; i < ROAD_TILE_COUNT; i++) {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(11, 0.18, TILE_DEPTH), roadBaseMat);
    base.position.set(0, -0.08, 0);
    tile.add(base);

    const leftLaneFloor = new THREE.Mesh(new THREE.BoxGeometry(2.65, 0.05, TILE_DEPTH - 0.25), laneFloorMat);
    leftLaneFloor.position.set(-3, 0.02, 0);
    tile.add(leftLaneFloor);

    const middleLaneFloor = new THREE.Mesh(new THREE.BoxGeometry(2.65, 0.05, TILE_DEPTH - 0.25), laneFloorMat);
    middleLaneFloor.position.set(0, 0.025, 0);
    tile.add(middleLaneFloor);

    const rightLaneFloor = new THREE.Mesh(new THREE.BoxGeometry(2.65, 0.05, TILE_DEPTH - 0.25), laneFloorMat);
    rightLaneFloor.position.set(3, 0.02, 0);
    tile.add(rightLaneFloor);

    const leftDivider = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.09, TILE_DEPTH - 0.6), cyanMat);
    leftDivider.position.set(-1.5, 0.14, 0);
    tile.add(leftDivider);

    const rightDivider = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.09, TILE_DEPTH - 0.6), cyanMat);
    rightDivider.position.set(1.5, 0.14, 0);
    tile.add(rightDivider);

    const leftEdge = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.14, TILE_DEPTH - 0.4), purpleMat);
    leftEdge.position.set(-5.25, 0.18, 0);
    tile.add(leftEdge);

    const rightEdge = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.14, TILE_DEPTH - 0.4), purpleMat);
    rightEdge.position.set(5.25, 0.18, 0);
    tile.add(rightEdge);

    const centerDash = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.11, 2.2), goldMat);
    centerDash.position.set(0, 0.18, 0);
    tile.add(centerDash);

    if (i % 3 === 0) {
      const mandala = new THREE.Mesh(new THREE.TorusGeometry(0.55, 0.035, 12, 48), goldMat);
...
function shoot() {
  ...
  if (missionText) missionText.textContent = "Shoot fired";
}
...
function droneShootEMP() {
  ...
  if (missionText) missionText.textContent = "EMP fired by drone";
}
...
function destroyDrone() {
  ...
  if (missionText) missionText.textContent = "Drone destroyed +5 Surya Shards";
  if (abilityText) abilityText.textContent = "Drone Down";
...
function respawnDrone() {
  ...
  if (missionText) missionText.textContent = "New drone incoming";
}
...
function damagePlayer(amount) {
  ...
  if (missionText) missionText.textContent = "EMP hit! Core damaged";
...
function dash() {
  ...
  if (abilityText) abilityText.textContent = "Surya Dash Activated";
